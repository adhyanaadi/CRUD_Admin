...
chartjs.register(
  ...
  const handleDelete = async (index) => {
    ...
    try {
      ...
      const response = await fetch(`${SERVER_URL}/api/customerHealth/${customerRecordToDelete.date}`, {
        method: 'DELETE',
      });
  ...
  const fetchAllRecords = async()=> {
    try {
      const response = await fetch(`${SERVER_URL}/api/customerHealth/${email}`);
      if (response.ok) {
...
const fetchCustomerData = async (email, setLoading, setCustomerData) => {
  try {
    const response = await fetch(`${SERVER_URL}/api/customerGet/${email}`);
    if (!response.ok) {
...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editMode) {
      ...
      try {
        const response = await fetch(`${SERVER_URL}/api/customerHealth/${editHealthDate}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
